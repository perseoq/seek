...
class DeepSeekCLI:
    ...
    def safe_print(self, text):
        """Maneja la impresión de caracteres Unicode en Windows"""
        try:
            print(text)
    ...
    def save_output(self, content, args):
        ...
        base_filename = args.name if args.name else f"deepseek_output_{timestamp}"
        ...
        if args.m:
            ...
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(content)
                self.safe_print(f"✓ Output guardado como Markdown: {filename}")
            except Exception as e:
                self.safe_print(f"✗ Error al guardar Markdown: {str(e)}")
        ...
        if args.f:
            ...
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(content)
                self.safe_print(f"✓ Output guardado como texto: {filename}")
            except Exception as e:
                self.safe_print(f"✗ Error al guardar texto: {str(e)}")
            ...
            for i, (block_type, lang, code) in enumerate(code_blocks, 1):
                if block_type == 'code' and lang:
                    ...
                    try:
                        with open(filename, 'w', encoding='utf-8') as f:
                            f.write(code)
                        self.safe_print(f"✓ Código guardado como: {filename}")
                    except Exception as e:
                        self.safe_print(f"✗ Error al guardar código: {str(e)}")
        
        if args.gz or args.zip:
            files_to_compress = []
            ...
            if args.c:
                for i, (block_type, lang, _) in enumerate(self.extract_code_blocks(content), 1):
                    if block_type == 'code' and lang:
                        ext = {
                            'python': 'py',
                            'javascript': 'js',
                    ...
                    with tarfile.open(tar_filename, "w:gz") as tar:
                        for file in files_to_compress:
                            if os.path.exists(file):
                                tar.add(file)
                                os.remove(file)
                    self.safe_print(f"✓ Archivos comprimidos como: {tar_filename}")
                except Exception as e:
                    self.safe_print(f"✗ Error al comprimir tar.gz: {str(e)}")
                    ...
                    with zipfile.ZipFile(zip_filename, 'w') as zipf:
                        for file in files_to_compress:
                            if os.path.exists(file):
                                zipf.write(file)
                                os.remove(file)
                    self.safe_print(f"✓ Archivos comprimidos como: {zip_filename}")
                except Exception as e:
                    self.safe_print(f"✗ Error al comprimir zip: {str(e)}")
...
Opciones de output (requieren -o):
  -m              Guardar como archivo markdown (.md)
  -f              Guardar como archivo de texto (.txt)
  -c              Extraer bloques de código a archivos
...
def main():
    ...
    parser.add_argument('-i', '--input', required=True, help='Input text for DeepSeek')
    ...
    parser.add_argument('-m', action='store_true', help='Save as markdown (requires -o)')
    parser.add_argument('-f', action='store_true', help='Save as text file (requires -o)')
    parser.add_argument('-c', action='store_true', help='Extract code blocks to files (requires -o)')
    ...
    if args.output:
        if not any([args.m, args.f, args.c]):
